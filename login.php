							...
							<?php
								if (isset($_POST['login'])) 
								{
									...
									if ($yangcocok > 0) 
									{
										...
										$ambil_user =$koneksi->query("SELECT * from pelanggan WHERE email_pelanggan='$user' AND password_pelanggan ='$pass' ");
										$yangcocok =$ambil_user->num_rows;

										if($yangcocok > 0){
											$ambil_user = $ambil_user->fetch_assoc();
											$_SESSION["nama"] = $ambil_user['nama_pelanggan']; 
											
											echo "<div class='alert alert-info'>Login sukses</div>";
											echo "<meta http-equiv='refresh' content='1;url=beranda_user.php'>";
										}else{
